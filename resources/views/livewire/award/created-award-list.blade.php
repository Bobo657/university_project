<div>
        ...
        <div class="rounded-lg bg-slate-150 p-4 dark:bg-navy-700">
            <div class="flex justify-between space-x-1">
                <p class="text-xl font-semibold text-slate-700 dark:text-navy-100">
                    {{ number_format($awards->sum('ballots_count')) }}
                </p>
                <svg
            ...
            <p class="mt-1 text-xs+">Total Nominees</p>
        </div>
        <div class="rounded-lg bg-slate-150 p-4 dark:bg-navy-700">
    ...
    @livewire('award.create-new-award-form')
            ...
            <div class="is-scrollbar-hidden min-w-full overflow-x-auto">
                ...
                <tbody>
                    ...
                    <tr class="border-y border-transparent border-b-slate-200 dark:border-b-navy-500">
                        ...
                        {{ number_format($award->ballots_count) }}
                        </td>
                        <td class="whitespace-nowrap px-4 py-3 sm:px-5 text-center">
                            {{ number_format($award->votes_count) }}
                        </td>
                        <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                            <button wire:click="$emit('addNominee', {{$award->id}})" class="btn h-8 w-8 text-info rounded-full p-0 hover:bg-slate-300/20 focus:bg-slate-300/20 active:bg-slate-300/25 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20 dark:active:bg-navy-300/25">
                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24">
                                    <path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z" fill="currentColor"/>
                                  </svg>
                            </button>
                            @if($award->ballots_count == 0)
                            <button wire:click="confirmDelete({{$award->id}})" class="btn h-8 w-8 text-error rounded-full p-0 hover:bg-slate-300/20 focus:bg-slate-300/20 active:bg-slate-300/25 dark:hover:bg-navy-300/20 dark:focus:bg-navy-300/20 dark:active:bg-navy-300/25">
